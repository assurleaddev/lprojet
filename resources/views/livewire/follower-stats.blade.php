<div class="mt-1 flex items-center gap-2 text-[15px]" wire:poll.5s="refreshCounts">
    ...
    <a href="{{ route('vendor.followers', $user) }}" class="hover:underline" style="color: var(--brand)">
        <span wire:key="followers-{{ $followersCount }}">{{ $followersCount }}</span> followers
    </a>,
    <a href="{{ route('vendor.following', $user) }}" class="hover:underline" style="color: var(--brand)">
