...
<div class="relative rounded-md shadow-sm border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4 py-5">
    ...
    <div class="flex items-start justify-between mb-4">
        <div>
            ...
            <p class="text-2xl font-bold text-green-600">{{ $total }} MAD</p>
        </div>
         ...
         x-data="{
             ...
             init() {
                 this.chart = new ApexCharts(
                     this.$refs.chartEl,
                     incomeChartOptions(@js($labels), @js($series), '#22c55e', @js(__('Commission')))
                 );
                 this.chart.render();
             },
             updateChart(labels, series) {
                 this.chart.updateOptions({ xaxis: { categories: labels } }, false, false);
                 this.chart.updateSeries([{ name: @js(__('Commission')), data: series }]);
             }
         }"
         x-on:income-chart-commission-update.window="updateChart($event.detail.labels, $event.detail.series)">
