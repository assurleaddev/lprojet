    ...
    <div class="flex items-start justify-between mb-4">
        <div>
            ...
            <p class="text-2xl font-bold text-amber-500">
                {{ number_format(array_sum($income_data_protection['protection']), 2) }} MAD
            </p>
        </div>
        <div class="flex items-center gap-2">
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                    ...
                    {{ $filterLabels[$income_period_protection] }}
                ...
                <div x-show="open" @click.outside="open = false" x-transition
                    ...
                    <ul class="py-1 text-xs text-gray-700 dark:text-gray-200">
                        ...
                        <li>
                            <a href="{{ request()->fullUrlWithQuery(['income_period_protection' => $value]) }}"
                               class="block px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-600 {{ $income_period_protection === $value ? 'bg-amber-50 dark:bg-gray-600 font-semibold text-amber-700 dark:text-amber-300' : '' }}">
                                {{ $label }}
                            </a>
                        </li>
                        @endforeach
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    new ApexCharts(el, incomeChartOptions(
        @json($income_data_protection['labels']),
        @json($income_data_protection['protection']),
        '#f59e0b',
        '{{ __('Buyer Protection') }}'
    )).render();
});
