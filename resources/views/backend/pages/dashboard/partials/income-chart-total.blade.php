    ...
    <div class="flex items-start justify-between mb-4">
        <div>
            ...
            <p class="text-2xl font-bold" style="color:#635BFF;">
                {{ number_format(array_sum($income_data_total['total']), 2) }} MAD
            </p>
        </div>
        <div class="flex items-center gap-2">
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                    ...
                    {{ $filterLabels[$income_period_total] }}
                ...
                <div x-show="open" @click.outside="open = false" x-transition
                    ...
                    <ul class="py-1 text-xs text-gray-700 dark:text-gray-200">
                        ...
                        <li>
                            <a href="{{ request()->fullUrlWithQuery(['income_period_total' => $value]) }}"
                               class="block px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-600 {{ $income_period_total === $value ? 'bg-indigo-50 dark:bg-gray-600 font-semibold text-indigo-700 dark:text-indigo-300' : '' }}">
                                {{ $label }}
                            </a>
                        </li>
                        @endforeach
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    new ApexCharts(el, incomeChartOptions(
        @json($income_data_total['labels']),
        @json($income_data_total['total']),
        '#635BFF',
        '{{ __('Total Income') }}'
    )).render();
});
