    ...
    <div class="flex items-start justify-between mb-4">
        <div>
            ...
            <p class="text-2xl font-bold text-red-500">
                {{ number_format(array_sum($income_data_verification['verification']), 2) }} MAD
            </p>
        </div>
        <div class="flex items-center gap-2">
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open"
                    ...
                    {{ $filterLabels[$income_period_verification] }}
                ...
                <div x-show="open" @click.outside="open = false" x-transition
                    ...
                    <ul class="py-1 text-xs text-gray-700 dark:text-gray-200">
                        ...
                        <li>
                            <a href="{{ request()->fullUrlWithQuery(['income_period_verification' => $value]) }}"
                               class="block px-3 py-1.5 hover:bg-gray-100 dark:hover:bg-gray-600 {{ $income_period_verification === $value ? 'bg-red-50 dark:bg-gray-600 font-semibold text-red-700 dark:text-red-300' : '' }}">
                                {{ $label }}
                            </a>
                        </li>
                        @endforeach
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    new ApexCharts(el, incomeChartOptions(
        @json($income_data_verification['labels']),
        @json($income_data_verification['verification']),
        '#ef4444',
        '{{ __('Verification') }}'
    )).render();
});
