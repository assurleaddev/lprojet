...
<x-card>
        ...
        @can('marketplace.products.approve')
            <div class="mb-4">
                ...
                <select id="status" name="status" class="form-input">
                    <option value="pending" @selected(old('status', $product->status ?? 'pending') == 'pending')>Pending</option>
    ...
    <div class="p-6 flex justify-between items-center">
        <a href="{{ route('admin.marketplace.products.index') }}" class="text-sm font-medium text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white">Cancel</a>
    ...
    <x-card>
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            ...
            <div class="mb-4">
                ...
                <input type="number" id="price" name="price" value="{{ old('price', $product->price ?? '') }}" class="form-input" required step="0.01">
                ...
                <div id="image-gallery" class="grid grid-cols-3 gap-4">
                    @foreach($product->images as $image)
                        <div class="relative group" id="image-container-{{ $image->id }}">
                            <img src="{{ asset('storage/' . $image->path) }}" alt="Product Image" class="w-full h-auto rounded-md object-cover">
                            <button type="button" class="delete-image-btn absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 leading-none opacity-0 group-hover:opacity-100 transition-opacity" data-url="{{ route('admin.marketplace.products.images.destroy', $image->id) }}">
