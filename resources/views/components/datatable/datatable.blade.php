                    ...
                    @forelse ($data as $item)
                             @if (isset($item->children) && !empty($item->children))
                                ...
                                <tr
                                    ...
                                    class="{{ $loop->index + 1 != count($data) ? 'table-tr' : '' }} parent-row"
                                    data-id="{{ $item->id }}"
                                    data-parent="0"
                                >
                                    {{-- checkbox (policy-based). No Alpine/JS handlers. --}}
                                    {{-- @if($enableCheckbox ?? true)
                                        @can('delete', $item)
                                    ...
                                    @foreach($headers ?? [] as $header)
                                        <td class="table-td {{ isset($header['align']) ? 'text-' . $header['align'] : '' }}">
                                            ...
                                            {{-- toggle only on the first column if there are children --}}
                                            @if($loop->first && !empty($item->children) && count($item->children) > 0)
                                                <button type="button"
                                ...
                                {{-- children (initially hidden via .is-hidden class); JS toggles by data-parent --}}
