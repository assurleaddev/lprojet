...
<div class="space-y-6" x-data="{
    ...
    <div class="rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="px-5 py-4 sm:px-6 sm:py-5 flex flex-col md:flex-row justify-between items-center gap-3">
            ...
            <div class="flex items-center gap-3">
                ...
                @if ($enableSearchbar)
                    @if ($customSeachForm)
                        {!! $customSeachForm !!}
                    @else
                        <x-datatable.searchbar :placeholder="$searchbarPlaceholder" :enableLivewire="$enableLivewire" />
                    @endif
                @endif
            ...
            <div class="flex items-center gap-3" @if ($enableLivewire) wire:ignore @endif>
                <div class="flex items-center gap-2">
                    @if ($enableBulkActions)
                        @if ($customBulkActions)
                            {!! $customBulkActions !!}
                        @else
                            <div class="relative flex items-center justify-center" x-show="selectedItems.length > 0"
                                x-data="{ open: false }">
                            ...
                            <div x-cloak x-show="bulkDeleteModalOpen" x-transition.opacity.duration.200ms
                                ...
                                <div x-show="bulkDeleteModalOpen"
                                    ...
                                    <div
                                        ...
                                        <h3 id="bulk-delete-modal-title"
                                            class="font-semibold tracking-wide text-gray-700 dark:text-white">
                                    ...
                                    <div
                                        class="flex items-center justify-end gap-3 border-t border-gray-100 p-4 dark:border-gray-800">
                                        @if ($this->getBulkDeleteAction()['url'])
                                            <form id="bulk-delete-form"
                                                action="{{ $this->getBulkDeleteAction()['url'] }}" method="POST">
                                        ...
                                        @else
                                            ...
                                            <button type="button" @click="bulkDeleteModalOpen = false"
                                                @if ($enableLivewire) wire:click="bulkDelete"
                ...
                @if ($enableNewResourceLink)
                    ...
                    @elseif(
                        $newResourceLinkPermission &&
                            ($newResourceLinkRouteUrl || $newResourceLinkRouteName) &&
                            auth()->user()->can($newResourceLinkPermission))
                        <a href="{{ $newResourceLinkRouteUrl ?: route($newResourceLinkRouteName) }}"
        ...
        <div class="table-responsive">
                    ...
                    <tr class="table-tr">
                        ...
                        @foreach ($headers ?? [] as $header)
                            <th @isset($header['width']) width="{{ $header['width'] }}" @endisset
                                ...
                                <div
                                    class="flex w-full items-center {{ isset($header['align']) ? 'justify-' . ($header['align'] === 'right' ? 'end' : 'start') : '' }}">
                                    {{ __($header['title']) }}

                                    @if ($header['sortable'] ?? false)
                                        <button
                                            @if ($enableLivewire) wire:click="sortBy('{{ $header['sortBy'] ?? strtolower(str_replace(' ', '_', $header['title'])) }}')"
                                    @else
                                        @php
                                            $sortKey = $header['sortBy'] ?? strtolower(str_replace(' ', '_', $header['title']));
                                            $nextDirection = ($sort === $sortKey && $direction === 'asc') ? 'desc' : 'asc';
                                        @endphp
                                        onclick="window.location='{{ request()->fullUrlWithQuery(['sort' => $sortKey, 'direction' => $nextDirection]) }}'" @endif
                                            class="ml-1 focus:outline-none">
                                            @if (isset($header['sortBy']) && $sort === $header['sortBy'] && $direction === 'asc')
                                                <iconify-icon icon="lucide:sort-asc"
                                                    class="text-primary"></iconify-icon>
                                            @elseif(isset($header['sortBy']) && $sort === $header['sortBy'] && $direction === 'desc')
                                                <iconify-icon icon="lucide:sort-desc"
                                                    class="text-primary"></iconify-icon>
                                            @else
                ...
                <tbody>
                    @forelse ($data as $item)
                        @if (isset($item->children) && !empty($item->children))
                            {{-- start custom --}}
                            <tr class="{{ $loop->index + 1 != count($data) ? 'table-tr' : '' }} parent-row"
                                data-id="{{ $item->id }}" data-parent="0">
                                    ...
                                    <td class="table-td table-td-checkbox" wire:ignore>
                                        <input type="checkbox" class="item-checkbox form-checkbox"
                                            ...
                                            if ($event.target.checked) {
                                                if (!selectedItems.includes({{ $item->id }})) selectedItems.push({{ $item->id }});
                                            } else {
                                ...
                                @foreach ($headers ?? [] as $header)
                                    <td
                                        ...
                                        @php
                                            // Keep your original per-column rendering logic
                                            $pascalCaseId = collect(explode('_', $header['id']))
                                            ...
                                            if ($enableLivewire) {
                                                ...
                                                if (
                                                    isset($header['renderContent']) &&
                                                    is_string($header['renderContent'])
                                                ) {
                                        ...
                                        {{-- toggle only on the first visible column if there are children --}}
                                        @if ($loop->first && !empty($item->children))
                                            <button type="button"
                                                class="js-toggle mr-2 text-xs text-primary align-middle"
                                                data-id="{{ $item->id }}" data-state="collapsed"
                                                aria-expanded="false"
                            ...
                            {{-- Render children (hidden by JS initially) --}}
                                    ...
                                    <td
                                        ...
                                        @php
                                            $pascalCaseId = collect(explode('_', $header['id']))
                                                ->map(fn($part) => ucfirst($part))
                                                ->implode('');
                                            $content = isset($data[$loop->index][$header['id']])
                                                ? $data[$loop->index][$header['id']]
                                                : null;
                                            if ($enableLivewire) {
                                                ...
                                                if (
                                                    ...
                                                    // Auto-discovered method - `render[Id]Cell()`
                                                    $content = $this->{$autoDiscoverableMethodName}($item, $header);
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ count($headers ?? []) + ($enableCheckbox ?? true ? 1 : 0) }}"
            ...
            @if ($enablePagination ?? true)
                <div class="my-4 px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center gap-2">
                        <label for="perPage"
                            class="text-sm text-gray-600 dark:text-gray-300">{{ __('Per page') }}</label>
                        <select id="perPage" wire:model.live="perPage" class="form-control w-20">
