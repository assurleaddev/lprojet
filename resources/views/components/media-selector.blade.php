...
<div 
    ...
    x-data="mediaSelector('{{ $modalId }}', {{ json_encode($selectedFiles) }}, {{ $multiple ? 'true' : 'false' }}, {{ $existingMedia ? 'true' : 'false' }}, {{ $min }}, {{ $max }})"
>
...
@push('scripts')
<script>
    function mediaSelector(modalId, existingMedia = [], multiple = false, hasExistingMedia = false, min = 1, max = 1) {
        return {
            selectedMedia: [],
            modalId: modalId,
            multiple: multiple,
                ...
                window[`handleMediaSelection_${modalId}`] = (files) => {
                    if (this.multiple) {
                        if (files.length < this.min) {
                            alert(`You must select at least ${this.min} files.`);
                            return;
                        }
                        if (files.length > this.max) {
                            alert(`You can select at most ${this.max} files.`);
                            return;
                        }
