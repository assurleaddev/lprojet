                            ...
                            <div class="bg-blue-50 p-4 rounded-md border border-blue-200">
                                ...
                                <form action="{{ route('settings.security.email.verify') }}" method="POST" class="flex gap-2">
                                    ...
                                    <input type="text" name="code" placeholder="Enter code"
                                        class="flex-1 border border-gray-300 rounded-lg p-2.5 shadow-sm focus:border-[var(--brand)] focus:ring-[var(--brand)]">
                                    <button type="submit"
                                        class="bg-[var(--brand)] text-white px-6 py-2 rounded font-medium hover:opacity-90">Verify</button>
                                </form>
                            </div>
                        @else
                            <div>
                    ...
                    <form action="{{ route('settings.security.password.update') }}" method="POST"
                        ...
                        <div>
                            ...
                            <input type="password" name="current_password"
                                class="w-full border border-gray-300 rounded-lg p-2.5 shadow-sm focus:border-[var(--brand)] focus:ring-[var(--brand)]">
                        ...
                        <button type="submit"
                            class="bg-[var(--brand)] text-white px-6 py-2 rounded font-medium hover:opacity-90">Update
                            Password</button>
                    </form>
                </div>
                        ...
                        <div class="mt-4 bg-blue-50 p-4 rounded-md border border-blue-200">
                            ...
                            <form action="{{ route('settings.security.2fa.verify') }}" method="POST" class="flex gap-2">
                                ...
                                <input type="text" name="code" placeholder="Enter code"
                                    class="flex-1 border border-gray-300 rounded-lg p-2.5 shadow-sm focus:border-[var(--brand)] focus:ring-[var(--brand)]">
                                <button type="submit"
                                    class="bg-[var(--brand)] text-white px-6 py-2 rounded font-medium hover:opacity-90">Verify &
                                    Enable</button>
                            </form>
                        </div>
                    @endif
                            ...
                            <div class="flex items-center justify-between p-4 border border-gray-200 rounded-md">
                                <div>
                                    <div class="font-medium text-gray-900">
                                        ...
                                        @if($session->id === request()->session()->getId())
                                            <span class="text-[var(--brand)] text-xs ml-2">(Current Device)</span>
                                        @endif
                                    </div>
                                ...
                                @if($session->id !== request()->session()->getId())
                                    <form action="{{ route('settings.security.logout_session', $session->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="text-sm font-medium text-[var(--brand)] hover:underline">Log
                                            out</button>
                                    </form>
                                @endif
                            </div>
