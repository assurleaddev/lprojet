...
@section('content')
    <div class="mx-auto max-w-[800px] px-4 py-8">
        <h1 class="text-2xl font-bold mb-6 text-gray-800">{{ __('Edit Your Item') }}</h1>
        ...
        <form action="{{ route('items.update', $product) }}" method="POST" enctype="multipart/form-data" id="item-form">
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-4 min-h-[200px] flex flex-wrap gap-4"
                    ...
                    <div class="w-32 h-32 flex items-center justify-center border border-gray-500 rounded cursor-pointer hover:bg-gray-100 transition-colors relative"
                        ...
                        <div class="text-center">
                            ...
                            <span class="text-gray-600 font-semibold text-sm">{{ __('Add more') }}</span>
                        </div>
                    </div>
            ...
            <div class="bg-white p-6 rounded-lg border border-gray-200 mb-6 space-y-6">
                ...
                @php $selectedFabrics = old('fabric', $product->fabric ?? []); @endphp
                <div x-data="{ checked: {{ count($selectedFabrics) }} }">
            ...
            <button type="submit"
                class="w-full bg-gray-900 text-white font-bold py-3 rounded-md hover:bg-gray-800 transition-colors">{{ __('Update Product') }}</button>
        </form>
    </div>
                                ...
                                attributes.forEach(attr => {
                                    ...
                                    if (attr.type === 'color') {
                                        ...
                                        (attr.options || []).forEach(option => {
                                            ...
                                            labelEl.innerHTML = `
                                                                                        <input type="radio" name="options[${attr.id}]" value="${option.id}" ${isSelected ? 'checked' : ''} class="form-radio text-gray-700 focus:ring-gray-500">
