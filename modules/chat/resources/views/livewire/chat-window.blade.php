<div class="flex flex-col h-full" 
    ...
    {{-- 1. Header --}}
    <div class="px-4 md:px-6 py-4 border-b dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm flex-shrink-0">
        @if($this->conversation)
            ...
            <div class="flex items-center justify-between">
                {{-- Left: Back button (mobile only) + User Info --}}
                <div class="flex items-center space-x-3">
                    <button class="md:hidden -ml-1 mr-1 p-1 text-gray-500 hover:text-gray-800"
                        @click="$dispatch('back-to-inbox')">
            ...
            <div class="mt-3 flex items-center gap-3 border-t pt-3">
                <div class="flex-shrink-0">
                    <img src="{{ $this->conversation->product->getFeaturedImageUrl('preview') }}"
                        alt="{{ $this->conversation->product->name }}" class="w-10 h-14 object-cover rounded-md bg-gray-100">
                </div>

                <div class="flex-1 min-w-0">
                    <h4 class="font-medium text-gray-900 text-sm truncate">{{ $this->conversation->product->name }}</h4>
                    ...
                    <p class="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                        {{ number_format($inclPrice, 2) }} MAD incl.
        ...
        @if($this->conversation)
            ...
            @forelse($messages as $key => $message)
                ...
                @if($messageType === 'platform_broadcast')
                    ...
                    @php
                        $meta = is_array($messageData->metadata) ? (object) $messageData->metadata : $messageData->metadata;
                        $senderAvatarUrl = $messageData->user['avatar_url'] ?? null;
                        ...
                        <div class="w-full max-w-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm">
                            ...
                            <div class="p-4 space-y-2">
                                <div class="flex items-center gap-2">
                                    @if($senderAvatarUrl)
                                        <img src="{{ $senderAvatarUrl }}" alt="{{ $appName }}" class="w-7 h-7 rounded-full object-cover shrink-0">
                                    @else
                                        <div class="w-7 h-7 rounded-full bg-gray-900 flex items-center justify-center text-white font-bold text-xs shrink-0">{{ substr($appName, 0, 1) }}</div>
                                    @endif
                                    <span class="text-xs font-semibold text-gray-900 dark:text-white">{{ $appName }}</span>
                                ...
                                @if(!empty($meta->pre_text))
                                    <p class="text-xs text-gray-500 dark:text-gray-400 italic">{{ $meta->pre_text }}</p>
                                @endif
                                ...
                                @if(!empty($meta->after_text))
                                    <p class="text-xs text-gray-500 dark:text-gray-400 italic">{{ $meta->after_text }}</p>
                                @endif
                                ...
                                <div class="p-4 bg-blue-50 dark:bg-blue-900/20 text-center">
                                    ...
                                    @if(isset($latestOrder) && $latestOrder->carrier && $latestOrder->tracking_code)
                                        <button wire:click="openTrackingModal" wire:loading.attr="disabled"
                                            ...
                                            <span wire:loading.remove wire:target="openTrackingModal">{{ __('Track my order') }}</span>
                                            <span wire:loading wire:target="openTrackingModal">{{ __('Loading…') }}</span>
                                        </button>
                                    @endif
                                    ...
                                    @if(isset($messageData->attachment_type) && $messageData->attachment_type === 'image')
                                        <a href="{{ Storage::url($messageData->attachment_path) }}" target="_blank">
                                            <img src="{{ Storage::url($messageData->attachment_path) }}"
                                                class="rounded-lg max-w-full h-auto max-h-64 object-cover" alt="Attachment">
                                        </a>
                                    @else
                                        <a href="{{ Storage::url($messageData->attachment_path) }}" target="_blank"
